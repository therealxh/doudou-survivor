...
| 玩家受击无敌帧 | 0.5s | — |
| 怪接触伤害 | 5 / 次（受无敌帧限频） | — |
| 飞刀 伤害 / 冷却 / 射程 / 弹速 | 12 / 0.8s / 6m / 12 m/s | 每级 +5 伤害 |
| 大蒜 伤害 / 冷却 / 半径 | 5 / 0.5s / 2.5m | 每级 +3 伤害 |
| 手雷 伤害 / 冷却 / 射程 / 爆炸半径 | 15 / 3.5s / 8m / 3m | 每级 +5 伤害（Day 8 配平：冷却 3.5s、半径 3m） |
| 怪 HP / 移速 / 半径 | 10 / 2.2 m/s / 0.4m | HP 每分钟 ×1.15（Day 8 配平：1.1 → 1.15） |
| 刷怪间隔 | 1.0s 起 | 每分钟 −0.12s（Day 8 配平），下限 0.2s |
| 同屏上限 | 300 | — |
| 宝石经验值 | 1 | — |
| 升级需求 | 12 +（等级−1）×8（Day 8 配平：原 5+3） | — |
| 单局时长上限 | 10 分钟（到时按胜利结算） | — |

**升级池（Day 7 实装版，用户确认）**：
- 飞刀：伤害 +5（×3）、冷却 -0.1s（×3）、多重射击 +1 发（×2，扇形 12°）、满 3 级伤害后解锁 **进化：弩枪**（伤害 +10 / 射程 +3m / 间隔 -0.1s / 弹体放大）
- 大蒜：伤害 +3（×3）、半径 +0.5m（×2）
- 手雷：伤害 +5（×3）
- 被动：攻击 +10%（×3）、移速 +10%（×3）、生命上限 +20 并回复（×3）、拾取范围 +20%（×3）
- 获取型：大蒜/手雷**开局不携带**，通过升级三选一获得
